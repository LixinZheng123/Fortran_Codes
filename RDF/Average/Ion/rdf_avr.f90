...
write(*,*) '------------------------------'
write(*,*) 'Input file         : ', trim(filename)//'.pos ',trim(filename)//'.ion'
...
select case(io)
  case(1)
    ...
    open(unit=2, file=(trim(filename)//'.ion'), status='old')
...
do 
  ...
  !******************************************************
  !Read in .ion
  read(2,*,iostat=ierror) step,time,dummy,iIO
  !write(*,*) iIO
  if (ierror .lt. 0) then
    write(*,*) '  End of File Reached'
    write(*,fmt='(1X, "  Total number of Samples: ", I7)' )(ncount)
  ...
  if (atom(1) .eq. 1 .and. atom(2) .eq. 1) then
    ...
    enddo
    !loop1:do i=1,nsp(1)-1
    !  if (i .eq. iIO) cycle loop1
    !  loop2: do k=i+1,nsp(1)
    !    if (k .eq. iIO) cycle loop2
    !      call get_distance(rO(1:3,i),rO(1:3,k),d)
    !      nbin = nint(d*res)
    !      if (nbin .gt. binNum) nbin = binNum
    !      temgrt(nbin) = temgrt(nbin) + 2
...
do n=1,binNum-1,1
  ...
  gofr = grt(n)/(DBLE(ncount*vshell*(nsp(1)-1)*dens))
  sum=sum+gofr
  !
